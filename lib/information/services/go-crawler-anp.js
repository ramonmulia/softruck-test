...
function formatCities(cities) {
  ...
  cities.forEach(function(city) {
    ...
    city.stations.forEach(function(itemStation) {
      itemStation.forEach(function(station) {
        if (!uniqueStations.length) {
          ...
          var aux = uniqueStations.filter(item => item.name == station.name && item.address == station.address);
          if (!aux.length) {
            uniqueStations.push({
              name: station.name,
              address: station.address
    ...
    var auxStationFilter = [],
      auxItemGrouped = [];
    uniqueStations.forEach(function(station) {
      var stations = city.stations.forEach(function(itemStation) {
        var stationFilter = itemStation.filter(filter => filter.name === station.name && filter.address == station.address);
        if (stationFilter.length) {
          auxStationFilter.push(stationFilter[0]);
        }
      });
      if (auxStationFilter.length) {
        auxItemGrouped.push(auxStationFilter);
      }
      auxStationFilter = [];
    });
    ...
    auxItemGrouped.forEach(function(itemGrouped) {
      ...
      stationObj.prices = prices;
      stationArr.push(stationObj);
      pass = true;
      prices = [];
    });
    city.stations = stationArr;
    stationArr = [];
  });
  return cities;
}
